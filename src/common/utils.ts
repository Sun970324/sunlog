 ...
 * @returns 현재 디바이스의 타입
 */
export const getDeviceType = (req?: ServerSideRequestType): 'desktop' | 'mobile' => {
  ...
  const isUAMobile = mobileTypes.includes(userAgent?.device?.type ?? '');
  return isUAMobile || isCloudfrontMobile ? 'mobile' : 'desktop';
};
