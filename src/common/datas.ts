...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    decision:
      '보드를 만들 때마다 두 프로그램에 같이 돌려서 결과가 다르면 실패하는 테스트를 넣었습니다. 1:1과 파티, 점령전 컨트롤러 세 개는 같은 검사를 한 번에 받도록 묶었습니다. 중요한 검사는 전부 DB와 서버 함수로 옮겨 RLS 테이블 35개, SECURITY DEFINER 함수 135개로 구성했습니다. 정답과 관리자, 토큰 테이블은 정책을 0개로 두고 권한을 회수했습니다. Apple 영수증은 루트 인증서까지 서버에서 검증합니다. 동시 접속이 많지 않아 랭크 매칭은 피크타임 두 번으로 모았고 실력 차가 나도 뒤집을 수 있게 점령전 모드를 따로 만들었습니다.',
    result:
  ...
  },
  {
    ...
    summary:
      '숙면 유도 사운드 앱입니다. 음악 프로듀서와 앱과 음원을 서로 맞춰 가며 만들었습니다. 음원 끝에 3초를 비워 달라는 요구는 제가, 멜로디와 자연음, 주파수를 동시에 재생해 달라는 요구는 프로듀서가 내는 식이었습니다. 기획자, 디자이너와 함께 Flutter 코드베이스 하나로 iOS와 Android를 같이 출시했습니다.',
    problem:
    ...
    decision:
      '멜로디와 자연음, 수면 주파수를 트랙 세 개로 나눠서 멜로디가 끝나도 자연음은 계속 흐르게 했습니다. 그런데 오디오 인스턴스 세 개가 따로 놀아서 재생과 정지에 딜레이가 생겼습니다. ViewModel 하나가 세 개를 같이 관리하도록 묶었습니다. 회원과 음원, 좋아요 데이터는 Firebase로 설계하고 로그인은 OAuth 2.0으로 붙였습니다. 반려는 회원탈퇴 기능을 만들고 약관을 앱 안에서 볼 수 있게 해서 해결했습니다.',
    result:
  ...
  },
  {
    ...
    decision:
      'Next.js와 TypeScript, Tailwind CSS로 다시 짰습니다. 프로젝트마다 문제와 판단, 결과를 나눠 쓰고 숫자를 앞에 두었습니다. 스크린샷은 클릭하면 크게 보이고 방향키로 넘기고 Esc로 닫게 했습니다. 다크 모드는 시스템 설정을 따르다가 직접 바꾸면 그 값을 기억합니다.',
